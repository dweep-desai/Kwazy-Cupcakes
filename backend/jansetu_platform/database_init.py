...
def init_appointments_schema_on_startup(db):
    """Initialize appointments schema if tables don't exist."""
    try:
        if settings.DATABASE_URL.startswith('sqlite'):
            # Check if consultation_requests table exists
            result = db.execute(text("SELECT name FROM sqlite_master WHERE type='table' AND name='consultation_requests'"))
            if result.fetchone():
            ...
            if 'consultation_requests' in inspector.get_table_names():
                return
            schema_file = os.path.join(os.path.dirname(__file__), '..', 'schema', 'appointments_schema.sql')
        
        if not os.path.exists(schema_file):
            print(f"[WARNING] Appointments schema file not found at {schema_file}.")
            return
        
        print("[INFO] Initializing appointments schema...")
...
def init_service_providers_schema_on_startup(db):
    ...
    try:
        ...
        # Seed service providers data if using SQLite
        if settings.DATABASE_URL.startswith('sqlite'):
            try:
                from ..schema.seed_service_providers_sqlite import seed_service_providers
                seed_service_providers()
            except Exception as e:
                print(f"[WARNING] Failed to seed service providers: {e}")
